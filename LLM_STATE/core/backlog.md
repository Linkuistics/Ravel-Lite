...
### Runner-owned dream-baseline creation + build metadata in `--version`
...
Two coupled issues surfaced during debugging on 2026-04-21:

1. `LLM_STATE/ravel-orchestrator/` and `LLM_STATE/sub-D-coordination/`
   in the `../Ravel/` project had no `dream-baseline` file despite many
   reflect cycles. Root cause: the fallback `seed_dream_baseline_if_missing`
   was only wired into the `GitCommitReflect` handler, which coordinator
   plans structurally never reach (they pivot in work phase). A prior
   deletion (commit `46e5b0d`, picked up silently by `git add .`) had
   also wiped the file for `ravel-orchestrator` without restoring it.
2. `ravel-lite --version` printed only `ravel-lite 0.1.0`, making the
   "which binary is running?" question in the above investigation
   unanswerable from the binary alone.

**Fix:**

A. **Three-layer self-healing for `dream-baseline`** — belt and
   suspenders so no single layer being unreachable leaves a plan
   baseline-less:
   - `src/create.rs::run_create` writes it post-session (runner owns
     creation, not the LLM).
   - `src/state.rs::run_set_phase` seeds it on every LLM phase
     transition (covers coordinators + post-deletion recovery).
   - `src/phase_loop.rs::handle_script_phase(GitCommitReflect)` keeps
     the original seed call (redundant but cheap).
   - Seed value is now `0` unconditionally (the "never dreamed"
     sentinel), replacing the prior "current word count" behavior that
     delayed the first dream by `headroom` words on plans that
     pre-dated the fallback.
   - Removed the prose instruction from `defaults/create-plan.md` —
     dream-baseline is no longer an LLM responsibility.
   - One-shot data reset: `LLM_STATE/core/dream-baseline` 1019 → 0 so
     this plan also benefits (it had been stuck above threshold).

B. **Build metadata in version output** — added `build.rs` emitting
   `BUILD_TIMESTAMP`, `GIT_DESCRIBE`, `GIT_SHA` as `rustc-env` vars.
   Both `ravel-lite version` and `ravel-lite --version` now print
   `ravel-lite 0.1.0 (v0.1.0-2-g15c2c8c-dirty, built 2026-04-21T...)`.
   `.git/HEAD`, `.git/refs/heads`, `.git/refs/tags` registered as
   rerun-if-changed so the metadata doesn't stale across rebuilds.

C. **Version-tag workflow** — added `release.toml` configuring
   [`cargo-release`](https://github.com/crate-ci/cargo-release):
   `publish = false`, `push = false` (explicit `git push` required),
   `tag-name = "v{{version}}"`. Documented the flow in README under
   a new "Releasing" section.

**Results:**

- Files modified: `src/dream.rs`, `src/state.rs`, `src/create.rs`,
  `src/main.rs`, `defaults/create-plan.md`, `tests/integration.rs`,
  `LLM_STATE/core/dream-baseline`, `README.md`.
- Files created: `build.rs`, `release.toml`.
- New tests: `seed_writes_zero_when_baseline_missing_regardless_of_memory_size`
  (replaces the prior current-word-count test);
  `set_phase_seeds_dream_baseline_when_missing`;
  `set_phase_preserves_existing_dream_baseline`. Updated
  `git_commit_reflect_seeds_dream_baseline_when_missing` for the new
  seed value.
- `cargo test` — 44 pass. `cargo clippy --all-targets` — clean.
- Verified version output: `ravel-lite 0.1.0 (15c2c8c-dirty, built
  2026-04-21T06:54:56Z)` (no tag yet, `--always` fell back to SHA).

**Residual / hand-off:**

- **Ravel plans auto-heal on next `ravel-lite state set-phase` call**
  once the operator rebuilds and installs the binary
  (`cargo install --path .`). Until then they remain baseline-less.
  Not automatable from inside this commit.
- **Initial `v0.1.0` tag** — not created here. First `cargo release
  patch --execute` (or manual `git tag -a v0.1.0`) will seed it; until
  then `git describe --always` falls back to the short SHA and the
  version output is still informative.
- **`cargo-release` is not pre-installed.** README instructs
  `cargo install cargo-release` as a one-time step. Not a build dep.
- Broader hand-off: three of the existing backlog items (coordinator
  plans, hand-off rationale preservation, structured-data CLI research)
  all touch file-write contracts that this work re-homed into the
  runner. None of them block on this, but the "mechanical ops belong
  in the runner" principle now has additional precedent.
