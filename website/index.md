...
The phase cycle runs: `triage → git-commit-triage → work → git-commit-work → analyse-work → git-commit-analyse-work → reflect → git-commit-reflect → repeat`. Each git-commit phase writes an audit-trail commit. Ravel-Lite works cleanly in monorepo subtrees by scoping all git operations to the project subtree via pathspec. Versions are released with `cargo-release`; binaries self-identify their build commit via `ravel-lite version`.
