...
* `commits.yaml` — written by analyse-work and applied by `git-commit-work`. The findings inbox (`findings.yaml`) lives at the *context level*, not in the plan directory; reflect and triage append to it via `ravel-lite findings add` when an observation is out of scope for the current plan.
