...
Everything Ravel-Lite does ends up in one of these files. Nothing is embedded in the binary, nothing lives only in an agent's head: the next chapter walks through how to inspect these files from the command line while a cycle is running.
