...
Continue through analyse-work, reflect, dream-trigger-skipped, and triage. Triage is where the hand-off gets mined. Two options the triage prompt considers for each hand-off block:
...
If the hand-off raises something out of scope for *this* plan but worth attention elsewhere, triage records it as a finding via `ravel-lite findings add` instead. Findings land in the context-level inbox where the user processes them out of band — promoting one into a new plan, marking it `wontfix`, or filing an external bug. No phase auto-applies a finding to another plan.
