...
Scaffolds a *ravel-context* on disk: the user's single root holding configuration, the repo registry, and every plan. The shipped prompts and agent definitions are baked into the binary and read from the embedded set on every invocation, so `init` only writes the user-owned files (`config.lua` starter, empty `repos.yaml`, empty `findings.yaml`) plus the four override-point subdirs (`agents/`, `phases/`, `fixed-memory/`, `plans/`) and runs `git init` inside the context so plan-state changes can be committed independently of any target repo.

----
ravel-lite init [--config <path>] [--force]
----

`--config <path>`:: Target directory for the context. Path-optional; resolves the same precedence chain as every other subcommand (`--config` → `$RAVEL_LITE_CONFIG` → platform default). The directory may already exist (init is idempotent) or be missing (init creates it).

`--force`:: Prune retired paths from a previously-scaffolded context. Never overwrites user-owned files (`config.lua`, `repos.yaml`, `findings.yaml`). Every embedded default is shipped in the binary, not on disk, so there is nothing to refresh — `--force` is purely a layout-cleanup tool used after upgrades that retire a previously-shipped path.

`init` is the only lifecycle command that does not need the context directory to already exist; it is how the context comes into being. The context owns its own git history (initial empty commit on `main`) so plan-state changes can be committed independently of any target repo. After `init` runs, point subsequent commands at the same path via `RAVEL_LITE_CONFIG` or `--config` (or, for the platform default, no flag at all).

Re-running `init` is safe and idempotent: existing user files survive, `.git/` is left alone, and only missing pieces get filled in. See the link:config-and-prompts.html[Configuration and prompts] reference for the override-point layout in full.
