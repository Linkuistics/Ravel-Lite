...
Finding:: A cross-plan observation recorded in the context-level findings inbox by reflect or triage when the cycle surfaces something out of scope for the current plan but worth attention elsewhere. Findings are advisory and user-mediated — no phase auto-acts on them; the operator promotes, archives, or marks `wontfix` out of band. Authored via `ravel-lite findings add`. See the link:cross-plan-files.html[Cross-plan files] reference.
...
Hand-off:: A task's hand-off block — extra context attached to a task that triage may promote into a new top-level task or archive into memory. Cleared after triage acts on it. Hand-offs are within-plan; cross-plan observations land in the findings inbox instead.
...
Evidence grade:: One of `strong`, `medium`, or `weak`. Strong means concrete cross-references in both projects' surface metadata; weak means inferred from indirect signals. The discover pipeline writes the grade based on stage-2 analysis; downstream consumers (reflect, triage, the survey) surface higher-grade edges as more salient context.
